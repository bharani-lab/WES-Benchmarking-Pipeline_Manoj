...
samtools mpileup -go output/"$r1"/bowtie_samtools_"$r1".bcf -f "$Re1" output/"$r1"/"$r1"_bowtie_recal.bam
#bcf_to_VCF_converstion
bcftools call -vmO z -o output/"$r1"/bowtie_samtools_"$r1".vcf output/"$r1"/bowtie_samtools_"$r1".bcf
#Variant_Calling-Free_Bayes
freebayes -f -f "$Re1" -v output/"$r1"/bowtie_Freebayes_"$r1".vcf output/"$r1"/"$r1"_bowtie_recal.bam
...
N_SHARDS=10
mkdir -p "${LOGDIR}"
time seq 0 $((N_SHARDS-1)) | parallel --eta --halt 2 --joblog "${LOGDIR}/log" --res "${LOGDIR}" python bin/make_examples.zip  --mode calling --ref "$Re1" --reads output/"$r1"/"$r1"_bowtie_recal.bam --examples "${OUTPUT_DIR}/examples.tfrecord@${N_SHARDS}.gz" --task {}
...
vcftools --vcf output/"$r1"/bowtie_samtools_"$r1".vcf --remove-indels --recode --recode-INFO-all --out output/"$r1"/bowtie_Samtools_SNP_"$r1".vcf
...
vcftools --vcf output/"$r1"/bowtie_samtools_"$r1".vcf --keep-only-indels  --recode --recode-INFO-all --out output/"$r1"/bowtie_Samtools_Indels_"$r1".vcf
